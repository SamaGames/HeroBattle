...
{
	/**
	 * A particle effect which is displayed by exploding tnt and creepers: <ul> <li>It looks like a
	 * white cloud <li>The speed value influences the velocity at which the particle flies off
	 * </ul>
	 */
	EXPLOSION_NORMAL("explode", 0, -1, ParticleProperty.DIRECTIONAL),
	/**
	 * A particle effect which is displayed by exploding ghast fireballs and wither skulls: <ul>
	 * <li>It looks like a gray ball which is fading away <li>The speed value slightly influences
	 * the size of this particle effect </ul>
	 */
	EXPLOSION_LARGE("largeexplode", 1, -1),
	/**
	 * A particle effect which is displayed by exploding tnt and creepers: <ul> <li>It looks like a
	 * crowd of gray balls which are fading away <li>The speed value has no influence on this
	 * particle effect </ul>
	 */
	EXPLOSION_HUGE("hugeexplosion", 2, -1),
	/**
	 * A particle effect which is displayed by launching fireworks: <ul> <li>It looks like a white
	 * star which is sparkling <li>The speed value influences the velocity at which the particle
	 * flies off </ul>
	 */
	FIREWORKS_SPARK("fireworksSpark", 3, -1, ParticleProperty.DIRECTIONAL),
	/**
	 * A particle effect which is displayed by swimming entities and arrows in water: <ul> <li>It
	 * looks like a bubble <li>The speed value influences the velocity at which the particle flies
	 * off </ul>
	 */
	WATER_BUBBLE("bubble", 4, -1, ParticleProperty.DIRECTIONAL, ParticleProperty.REQUIRES_WATER),
	/**
	 * A particle effect which is displayed by swimming entities and shaking wolves: <ul> <li>It
	 * looks like a blue drop <li>The speed value has no influence on this particle effect </ul>
	 */
	WATER_SPLASH("splash", 5, -1, ParticleProperty.DIRECTIONAL),
	/**
	 * A particle effect which is displayed on water when fishing: <ul> <li>It looks like a blue
	 * droplet <li>The speed value influences the velocity at which the particle flies off </ul>
	 */
	WATER_WAKE("wake", 6, 7, ParticleProperty.DIRECTIONAL),
	/**
	 * A particle effect which is displayed by water: <ul> <li>It looks like a tiny blue square
	 * <li>The speed value has no influence on this particle effect </ul>
	 */
	SUSPENDED("suspended", 7, -1, ParticleProperty.REQUIRES_WATER),
	/**
	 * A particle effect which is displayed by air when close to bedrock and the in the void: <ul>
	 * <li>It looks like a tiny gray square <li>The speed value has no influence on this particle
	 * effect </ul>
	 */
	SUSPENDED_DEPTH("depthSuspend", 8, -1, ParticleProperty.DIRECTIONAL),
	/**
	 * A particle effect which is displayed when landing a critical hit and by arrows: <ul> <li>It
	 * looks like a light brown cross <li>The speed value influences the velocity at which the
	 * particle flies off </ul>
	 */
	CRIT("crit", 9, -1, ParticleProperty.DIRECTIONAL),
	/**
	 * A particle effect which is displayed when landing a hit with an enchanted weapon: <ul> <li>It
	 * looks like a cyan star <li>The speed value influences the velocity at which the particle
	 * flies off </ul>
	 */
	...
	/**
	 * A particle effect which is displayed by fire, minecarts with furnace and blazes: <ul> <li>It
	 * looks like a large gray cloud <li>The speed value influences the velocity at which the
	 * particle flies off </ul>
	 */
	...
	/**
	 * A particle effect which is displayed by blocks beneath a water source: <ul> <li>It looks like
	 * a blue drip <li>The speed value has no influence on this particle effect </ul>
	 */
	DRIP_WATER("dripWater", 18, -1),
	/**
	 * A particle effect which is displayed by blocks beneath a lava source: <ul> <li>It looks like
	 * an orange drip <li>The speed value has no influence on this particle effect </ul>
	 */
	DRIP_LAVA("dripLava", 19, -1),
	/**
	 * A particle effect which is displayed when attacking a villager in a village: <ul> <li>It
	 * looks like a cracked gray heart <li>The speed value has no influence on this particle effect
	 * </ul>
	 */
	...
	/**
	 * A particle effect which is displayed by mycelium: <ul> <li>It looks like a tiny gray square
	 * <li>The speed value has no influence on this particle effect </ul>
	 */
	TOWN_AURA("townaura", 22, -1, ParticleProperty.DIRECTIONAL),
	/**
	 * A particle effect which is displayed by note blocks: <ul> <li>It looks like a colored note
	 * <li>The speed value causes the particle to be colored green when set to 0 </ul>
	 */
	...
	/**
	 * A particle effect which is displayed by enchantment tables which are nearby bookshelves: <ul>
	 * <li>It looks like a cryptic white letter <li>The speed value influences the spread of this
	 * particle effect </ul>
	 */
	...
	/**
	 * A particle effect which is displayed by lava: <ul> <li>It looks like a spark <li>The speed
	 * value has no influence on this particle effect </ul>
	 */
	LAVA("lava", 27, -1),
	/**
	 * A particle effect which is currently unused: <ul> <li>It looks like a transparent gray square
	 * <li>The speed value has no influence on this particle effect </ul>
	 */
	FOOTSTEP("footstep", 28, -1),
	/**
	 * A particle effect which is displayed when a mob dies: <ul> <li>It looks like a large white
	 * cloud <li>The speed value influences the velocity at which the particle flies off </ul>
	 */
	...
	/**
	 * A particle effect which is displayed when snowballs hit a block: <ul> <li>It looks like a
	 * little piece with the snowball texture <li>The speed value has no influence on this particle
	 * effect </ul>
	 */
	SNOWBALL("snowballpoof", 31, -1),
	/**
	 * A particle effect which is currently unused: <ul> <li>It looks like a tiny white cloud
	 * <li>The speed value influences the velocity at which the particle flies off </ul>
	 */
	SNOW_SHOVEL("snowshovel", 32, -1, ParticleProperty.DIRECTIONAL),
	/**
	 * A particle effect which is displayed by slimes: <ul> <li>It looks like a tiny part of the
	 * slimeball icon <li>The speed value has no influence on this particle effect </ul>
	 */
	SLIME("slime", 33, -1),
	/**
	 * A particle effect which is displayed when breeding and taming animals: <ul> <li>It looks like
	 * a red heart <li>The speed value has no influence on this particle effect </ul>
	 */
	HEART("heart", 34, -1),
	/**
	 * A particle effect which is displayed by barriers: <ul> <li>It looks like a red box with a
	 * slash through it <li>The speed value has no influence on this particle effect </ul>
	 */
	BARRIER("barrier", 35, 8),
	/**
	 * A particle effect which is displayed when breaking a tool or eggs hit a block: <ul> <li>It
	 * looks like a little piece with an item texture </ul>
	 */
	ITEM_CRACK("iconcrack", 36, -1, ParticleProperty.DIRECTIONAL, ParticleProperty.REQUIRES_DATA),
	/**
	 * A particle effect which is displayed when breaking blocks or sprinting: <ul> <li>It looks
	 * like a little piece with a block texture <li>The speed value has no influence on this
	 * particle effect </ul>
	 */
	BLOCK_CRACK("blockcrack", 37, -1, ParticleProperty.DIRECTIONAL, ParticleProperty.REQUIRES_DATA),
	/**
	 * A particle effect which is displayed when falling: <ul> <li>It looks like a little piece with
	 * a block texture </ul>
	 */
	BLOCK_DUST("blockdust", 38, 7, ParticleProperty.DIRECTIONAL, ParticleProperty.REQUIRES_DATA),
	/**
	 * A particle effect which is displayed when rain hits the ground: <ul> <li>It looks like a blue
	 * droplet <li>The speed value has no influence on this particle effect </ul>
	 */
	WATER_DROP("droplet", 39, 8),
	/**
	 * A particle effect which is currently unused: <ul> <li>It has no visual effect </ul>
	 */
	ITEM_TAKE("take", 40, 8),
	/**
	 * A particle effect which is displayed by elder guardians: <ul> <li>It looks like the shape of
	 * the elder guardian <li>The speed value has no influence on this particle effect <li>The
	 * offset values have no influence on this particle effect </ul>
	 */
	...
	/**
	 * Represents the property of a particle effect <p/> This class is part of the <b>ParticleEffect
	...
	/**
	 * Represents the item data for the {@link ParticleEffect#ITEM_CRACK} effect <p/> This class is
	...
	/**
	 * Represents the color for the {@link ParticleEffect#NOTE} effect <p/> This class is part of
	 * the <b>ParticleEffect Library</b> and follows the same usage conditions
	 *
	 * @author DarkBlade12
	 * @since 1.7
	 ...
	 * newer version <p/> This class is part of the <b>ParticleEffect Library</b> and follows the
	...
	public static final class ParticlePacket
	{
		 ...
		 * <p/> <b>Note:</b> These fields only have to be initialized once, so it will return if
		 ...
		 * this library <p/> This class is part of the <b>ParticleEffect Library</b> and follows the
		 * same usage conditions
		 *
		 * @author DarkBlade12
		 * @since 1.5
		...
		/**
		 * Represents a runtime exception that is thrown if packet instantiation fails <p/> This
		 * class is part of the <b>ParticleEffect Library</b> and follows the same usage conditions
		 *
		 * @author DarkBlade12
		 * @since 1.4
		 */
		...
		/**
		 * Represents a runtime exception that is thrown if packet sending fails <p/> This class is
		 * part of the <b>ParticleEffect Library</b> and follows the same usage conditions
		 *
		 * @author DarkBlade12
		 * @since 1.4
